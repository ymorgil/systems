...
La documentación oficial de HashiCorp para Terraform es la referencia más fiable para cualquier duda concreta: la [guía de introducción a Terraform](https://developer.hashicorp.com/terraform/intro){:target="_blank"} explica el paradigma IaC y el flujo de trabajo básico; la [documentación del lenguaje HCL y sus bloques](https://developer.hashicorp.com/terraform/language){:target="_blank"} detalla la sintaxis de `resource`, `variable`, `module` y `output` vista en este apartado; y la [documentación sobre backends y gestión del estado](https://developer.hashicorp.com/terraform/language/backend){:target="_blank"} profundiza en cómo configurar un backend remoto con bloqueo. El resto de enlaces de referencia del módulo está recopilado en la página de Recursos. Como ejemplo práctico, [jose-emilio/terraform-on-aws](https://github.com/jose-emilio){:target="_blank"} despliega infraestructura en AWS Academy con Terraform, útil para contrastar con los ejemplos de este apartado.
